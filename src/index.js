...
const { Client, Collection, GatewayIntentBits, Events, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, PermissionFlagsBits, WebhookClient, StringSelectMenuBuilder, InteractionResponseFlags } = require('discord.js'); // <-- Se añade InteractionResponseFlags
...
client.once(Events.ClientReady, () => { console.log(`¡Listo! ${client.user.tag} está online.`); });
...
// =========================================================================================
// === GESTIÓN DE INTERACCIONES (CON CORRECCIÓN DE "EPHEMERAL") ===
// =========================================================================================
client.on(Events.InteractionCreate, async interaction => {
    try {
        ...
        if (interaction.isChatInputCommand()) {
            ...
            // --- LÓGICA DE BOTONES (con 'ephemeral' corregido) ---
            if (interaction.customId === 'request_manager_role_button') {
                ...
                if (!esAprobador) return interaction.reply({ content: 'No tienes permiso.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                ...
                if (!team) return interaction.reply({ content: 'Este equipo ya no existe.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                ...
                if (isAlreadyInTeam) {
                    ...
                    return interaction.reply({ content: `Ya perteneces al equipo **${isAlreadyInTeam.name}**.`, flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                }
                ...
                await interaction.reply({ content: `¡Felicidades! Te has unido a **${team.name}**.`, flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                ...
                const teamId = interaction.customId.split('_')[2];
                await interaction.reply({ content: 'Has rechazado la invitación.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                const team = await Team.findById(teamId);
                if (team) {
                    const manager = await client.users.fetch(team.managerId);
                ...
                if (!team) return interaction.reply({ content: 'Solo los mánagers registrados pueden invitar.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                ...
                if (!team) return interaction.reply({ content: 'Debes ser mánager o capitán de un equipo.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                const memberIds = [...team.captains, ...team.players];
                if (memberIds.length === 0) return interaction.reply({ content: 'Tu equipo no tiene miembros para gestionar.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                const memberOptions = [];
                ...
                if (memberOptions.length === 0) return interaction.reply({ content: 'No se encontraron los miembros de tu equipo en el servidor.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                ...
                await interaction.reply({ content: 'Selecciona un miembro de tu plantilla:', components: [new ActionRowBuilder().addComponents(selectMenu)], flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
            } else if (interaction.customId.startsWith('promote_player_')) {
                const team = await Team.findOne({ guildId: interaction.guildId, managerId: interaction.user.id });
                if (!team) return interaction.reply({ content: 'Solo el mánager puede ascender.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                ...
                const team = await Team.findOne({ guildId: interaction.guildId, managerId: interaction.user.id });
                if (!team) return interaction.reply({ content: 'Solo el mánager puede degradar.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                const targetId = interaction.customId.split('_')[2];
                ...
                const team = await Team.findOne({ guildId: interaction.guildId, $or: [{ managerId: interaction.user.id }, { captains: interaction.user.id }] });
                if (!team) return interaction.reply({ content: 'No tienes permiso para expulsar.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                const targetId = interaction.customId.split('_')[2];
            ...
            if (interaction.customId === 'roster_management_menu') {
                ...
                await interaction.reply({ content: `Gestionando a **${targetMember.user.username}**:`, components: [row], flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
            }
            ...
            if (interaction.customId === 'manager_request_modal') {
                ...
                if (!approvalChannel) return interaction.reply({ content: 'Error: Canal de aprobaciones no encontrado.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                ...
                await interaction.reply({ content: 'Tu solicitud ha sido enviada para revisión.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                ...
                if (!originalRequestMessage) return interaction.reply({ content: 'No se pudo encontrar la solicitud original.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                ...
                if (existingTeam) return interaction.reply({ content: `Error: Ya existe un equipo llamado **${teamName}**.`, flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
                const isAlreadyManaged = await Team.findOne({ managerId: applicant.id });
                if (isAlreadyManaged) return interaction.reply({ content: `Error: Este usuario ya es mánager del equipo **${isAlreadyManaged.name}**.`, flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
        ...
        if (interaction.replied || interaction.deferred) await interaction.followUp({ content: 'Ha ocurrido un error al procesar tu solicitud.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
        else await interaction.reply({ content: 'Ha ocurrido un error al procesar tu solicitud.', flags: [InteractionResponseFlags.Ephemeral] }); // CORREGIDO
    }
});
